...
def main():
    ...
    try:
        ...
        if len(lstErrPar) > 0:
            print "Unknown command, try \"python run_taskscheduler.py --help\" for more information."
        elif len(lstPar) <> 1:
            print "Unknown command, try \"python run_taskscheduler.py --help\" for more information."
        else:
            if lstPar[0][0] == "--start":
                sys_log.SysLog(logFile, instance).writeLog("info", "The Task Scheduler Service is starting ...")
                daemon1.start()
                ...
                daemon1.stop()
                sys_log.SysLog(logFile, instance).writeLog("info", "The Task Scheduler Service is stopped")
            elif lstPar[0][0] == "--restart":
                sys_log.SysLog(logFile, instance).writeLog("info", "The Task Scheduler Service is restarting ...")
                daemon1.restart()
            elif lstPar[0][0] == "--status":
                daemon1.status()
            elif lstPar[0][0] == "--help":
                print "Usage: python run_taskscheduler.py {Options}"
                print "Options:"
                print "    --start:     Start the Task Scheduler Service"
                print "    --stop:      Stop the Task Scheduler Service"
                print "    --restart:   Restart the Task Scheduler Service"
                print "    --status:    Show the status of the Task Scheduler Service"
                print "    --help:      Show the help"
    except GetoptError, e:
        print e.msg + ", try \"run_taskscheduler.py --help\" for more information."
